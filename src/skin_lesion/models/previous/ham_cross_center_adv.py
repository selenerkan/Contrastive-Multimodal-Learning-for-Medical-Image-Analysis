...
class CrossCenterAdvModel(LightningModule):
    '''
    Uses ResNet for the image data, concatenates image and tabular data at the end
    '''

    def __init__(self, learning_rate=0.013, weight_decay=0.01, alpha_center=0.01, alpha_adv=0.5, dropout_rate=0):
        ...
        self.alpha_adv = (1-alpha_center)*alpha_adv
        self.alpha_cross_ent = (1-alpha_center-self.alpha_adv)
        ...
        # initiate losses
        # initialte adversarial loss
        # it will be used after embedding th eimage and tabular
        self.adversarial = AdversarialLoss(self.mlp)
    ...
    def forward(self, img, tab):
        ...
        return img.squeeze(0), tab.squeeze(1), out1, out2
    ...
    def training_step(self, batch, batch_idx):
        ...
        anchor_img, anchor_tab, embeddings, y_pred = self(img, tab)
        neg_img, neg_tab, neg_embeddings, _ = self(negative, negative_tab)

        # calculate adversarial loss
        # concat anchor image and anchor tab
        real_pair = torch.cat((anchor_img, anchor_tab), dim=1)
        fake_pair = torch.cat((anchor_img, neg_tab), dim=1)
        adv_loss = self.adversarial(real_pair, fake_pair)
        # cross entropy loss
        ...
        loss = self.alpha_cross_ent*cross_ent_loss + \
            self.alpha_center * center_loss + self.alpha_adv*adv_loss
        ...
        self.log('train_cross_ent_loss', cross_ent_loss,
                 on_epoch=True, on_step=False)
        self.log('train_adversarial_loss', adv_loss,
                 on_epoch=True, on_step=False)
        # log weights
        ...
        self.log('center_weight', self.alpha_center,
                 on_epoch=True, on_step=False)
        self.log('adversarial_weight', self.alpha_adv,
                 on_epoch=True, on_step=False)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_cross_ent_loss', cross_ent_loss,
                 on_epoch=True, on_step=False)
        self.log('val_adversarial_loss', adv_loss,
                 on_epoch=True, on_step=False)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log('test_cross_ent_loss', cross_ent_loss,
                 on_epoch=True, on_step=False)
        self.log('test_adversarial_loss', adv_loss,
                 on_epoch=True, on_step=False)
