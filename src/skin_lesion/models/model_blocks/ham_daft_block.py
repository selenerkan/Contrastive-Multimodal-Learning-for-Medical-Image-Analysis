...
from models.model_blocks.ham_resnet_block import conv2d, ConvBnReLU, ResBlock
    ...
    def rescale_features(self, feature_map, x_aux):
        ...
        if self.scale == self.shift:
            ...
            v_scale = v_scale.view(
                *v_scale.size(), 1, 1).expand_as(feature_map)
            v_shift = v_shift.view(
                *v_shift.size(), 1, 1).expand_as(feature_map)
            if self.scale_activation is not None:
                v_scale = self.scale_activation(v_scale)
        elif self.scale is None:
            v_scale = attention
