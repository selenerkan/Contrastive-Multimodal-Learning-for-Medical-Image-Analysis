...
class MultiLossLearnModel(LightningModule):
    '''
    Uses ResNet for the image data, concatenates image and tabular data at the end
    '''

    def __init__(self, learning_rate=0.013, weight_decay=0.01, alpha_center=0.01, triplet_ratio=0.5, dropout_rate=0):
        ...
        self.alpha_center = nn.Parameter(
            torch.tensor([0.01]).cuda(), requires_grad=True)
        self.alpha_cross_ent = nn.Parameter(
            torch.tensor([0.99]).cuda(), requires_grad=True)
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        ...
        # Normalize weights
        total_weight = self.alpha_cross_ent + self.alpha_center
        self.alpha_cross_ent.data = torch.clamp(
            self.alpha_cross_ent.data, min=1e-3) / total_weight
        self.alpha_center.data = torch.clamp(
            self.alpha_center.data, min=1e-3) / total_weight
